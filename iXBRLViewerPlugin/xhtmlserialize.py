...
class XHTMLSerializer:
    ...
    def qname_for_node(self, node: etree._Element) -> str:
        ...
        if qname.namespace is None:
            ...
            prefix = next(
                iter(sorted((p for p, ns in node.nsmap.items() if ns == qname.namespace), key=self.prefix_sort))
            )
        if prefix is None:
    ...
    def qname_for_attr(self, tag: etree._Element | str, nsmap: dict[str | None, str]) -> str:
        ...
        if qname.namespace == XML_NS:
            ...
            prefix = next(
                iter(
                    sorted(
                        (p for p, ns in nsmap.items() if ns == qname.namespace and p is not None), key=self.prefix_sort
                    )
                )
            )
        return f"{prefix}:{qname.localname}"
    ...
    def is_selfclosable(self, n: etree._Element | str) -> bool:
        qname = etree.QName(n)
        return qname.localname in XHTMLSerializer.SELF_CLOSABLE and (
            qname.namespace == XHTML_NS or (self.assume_xhtml and qname.namespace is None)
        )
