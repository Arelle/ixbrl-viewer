...
def qname_effect(prefix, namespaceURI, localName):
    return Mock(prefix=prefix, namespaceURI=namespaceURI, localName=localName)


def mrs_effect(dts, reltype):
    return Mock(fromModelObject=lambda source: [])
...
def mock_arelle():
    ...
    if "arelle" not in sys.modules:
        ...
        sys.modules["arelle.ModelValue"] = Mock(QName=qname_effect)
        sys.modules["arelle.ModelXbrl"] = Mock()
        sys.modules["arelle.PythonUtil"] = Mock()
        sys.modules["arelle.UrlUtil"] = Mock(isHttpUrl=lambda path: path.startswith("http"))
        sys.modules["arelle.ValidateXbrlCalcs"] = Mock(inferredDecimals=inferredDecimals_effect)
